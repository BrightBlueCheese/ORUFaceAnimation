...
def main():
    audio_info = mediainfo(args.audio_input)
    audio_duration = float(audio_info['duration'])

    # Write the video clip
    video_clip = VideoFileClip(args.video_input).subclip(0, audio_duration)
    modified_clip = video_clip.set_fps(args.video_fps) # set fps == 30
    ...
    cooked_video.write_videofile(args.video_output, temp_audiofile=False)
